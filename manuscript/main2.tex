...
\begin{itemize}
  ...
  \item \(\overline B_t<\infty\): upper bound on allowance purchases;
  \item \(\overline S_t<\infty\): upper bound on allowance sales;
  \item \(z_j^0\geq0\): initial physical usable inventory;
  \item \(\overline z_{jt}<\infty\): physical inventory capacity;
  ...
  \item \(\chi_j \in [0,1)\): memory modulation coefficient on
  fulfillment. This memory modulation coefficient is calibrated separately from observations linking beginning-of-period congestion to reductions in effective fulfillment capacity;
  \item \(\overline m_{jt}<\infty\): upper bound on absolute memory
  state;
...
\begin{equation}
  ...
  q_{jt}^{\omega},
  \qquad
  0\leq z_{jt}^{\omega}\leq\overline z_{jt}.
...
\begin{equation}
  ...
  \rho_jm_{jt}^{\omega},
  \qquad
  -\overline m_{jt}\leq m_{jt}^{\omega}\leq\overline m_{jt}.
...
\begin{equation}
  0\leq b_t^\omega\leq\overline B_t,
  \qquad
  0\leq s_t^\omega\leq\overline S_t.
...
\begin{remark}
The explicit bound \(\overline m_{jt}\) in
...
\(\overline m_{jt}\) should satisfy
\(\overline m_{jt}\geq K_t^{(\alpha)}\max_{\tau\leq t}\left\{\psi_j(\overline{r}_{j\tau}+\overline{d}_{j\tau})\right\}\)
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Piecewise-linear approximation of quadratic-penalty Progressive Hedging. Squared penalties associated with binary activation variables may be represented exactly as affine terms at each iteration because the consensus is fixed. Continuous disagreement penalties remain quadratic unless a declared approximation is used (in our PuLP implementation, a convex piecewise-linear secant approximation (PWL-PH) is employed for continuous variables to maintain MILP solvability).}
...
Because binary activation variables are duplicated across scenarios, the subproblems are inherently nonconvex mixed-integer programs. While a standard quadratic penalty would yield MIQPs, our exact PWL discretization preserves the MILP structure. Nonetheless, classical PH convergence theory does not guarantee global optimality. An implementation must track both the primal non-anticipativity residual $R_{\mathrm{NAC}}^r$ and the dual proxy residual $R_{\mathrm{proxy}}^r$:
\begin{align}
  R_{\mathrm{NAC}}^r
  &=
  \left[
    \sum_{n \in \mathcal{N}} \sum_{\omega \in \Omega(n)} p_\omega \|v_n^{\omega,r} - \bar{v}_n^r\|_2^2
  \right]^{1/2},
  \label{eq:nac-residual}\\
  R_{\mathrm{proxy}}^r
  &=
  \left[
    \sum_{n \in \mathcal{N}} \rho_{\mathrm{PH}}^2 \|\bar{v}_n^r - \bar{v}_n^{r-1}\|_2^2
  \right]^{1/2},
...
$A_t, \overline B_t, \overline S_t, p_t^b, p_t^s$ & $A_t=10, \overline B_t=50, \overline S_t=50, p_t^b=10, p_t^s=8$ \\
...
$z_{j0}, \overline m_{jt}$ & $z_{j0}=0, \overline m_{jt}=1000$ \\
...
\begin{table}[ht]\small
\centering
\caption{Scenario-level optimization outcomes. Note: First-stage binary activation costs ($f_k y_k$) are excluded from the reported scenario total $C^\omega$. The selected activation is $y_1=1, y_2=0$. Displayed emissions and cost components are rounded independently; identities are verified using the unrounded archived values.}
\begin{tabular}{lrrrrrrrrr}
\toprule
Scen & Prob & Transportation cost & Lost-Sales & Holding & Carbon trading & Total ($C^\omega$) & Emissions & Buy & Sell \\
\midrule
$\omega_{0}$ & 0.25 & 479.7 & 0.0 & 20.4 & -6.5 & \textbf{493.7} & 19.2 & 0.0 & 0.8 \\
$\omega_{1}$ & 0.25 & 292.2 & 0.0 & 20.4 & -66.5 & \textbf{246.2} & 11.7 & 0.0 & 8.3 \\
$\omega_{2}$ & 0.25 & 156.2 & 0.0 & 0.0 & -110.0 & \textbf{46.2} & 6.2 & 0.0 & 13.8 \\
$\omega_{3}$ & 0.25 & 93.8 & 0.0 & 0.0 & -130.0 & \textbf{-36.2} & 3.8 & 0.0 & 16.2 \\
\bottomrule
\end{tabular}
\label{tab:scenarios}
\end{table}
...
\midrule
$\omega_{0}$ & 1 & 9.2 & 10.0 & 0.0 & 0.8 & 0.0 \\
$\omega_{0}$ & 2 & 10.0 & 10.0 & 0.0 & 0.0 & 0.0 \\
$\omega_{1}$ & 1 & 9.2 & 10.0 & 0.0 & 0.8 & 0.0 \\
$\omega_{1}$ & 2 & 2.5 & 10.0 & 0.0 & 7.5 & 0.0 \\
$\omega_{2}$ & 1 & 2.5 & 10.0 & 0.0 & 7.5 & 0.0 \\
$\omega_{2}$ & 2 & 3.8 & 10.0 & 0.0 & 6.2 & 0.0 \\
$\omega_{3}$ & 1 & 2.5 & 10.0 & 0.0 & 7.5 & 0.0 \\
$\omega_{3}$ & 2 & 1.2 & 10.0 & 0.0 & 8.8 & 0.0 \\
\bottomrule
\end{tabular}
...
\midrule
$n_1$ & root & 1 & (15.0, 22.5) & 0.50 \\
$n_3$ & $n_{1}$ & 2 & (22.5, 33.8) & 0.50 \\
$n_4$ & $n_{1}$ & 2 & (7.5, 11.2) & 0.50 \\
$n_2$ & root & 1 & (5.0, 7.5) & 0.50 \\
$n_5$ & $n_{2}$ & 2 & (7.5, 11.2) & 0.50 \\
$n_6$ & $n_{2}$ & 2 & (2.5, 3.8) & 0.50 \\
\bottomrule
\end{tabular}
\label{tab:tree}
\end{table}
...
\midrule
Deterministic equivalent & 1 & 0 & 1340.60 & 0.01s & 0 & --- & 0.00\% \\
Recovered PWL-PH policy & 0 & 0 & Infeasible & 0.82s & 0.0320 & 20 & N/A \\
\bottomrule
\end{tabular}
...
\midrule
0 & 0.6058 & 13806.7741 & 0.06 & 0.00 & 0.0 \\
1 & 0.0320 & 213.9769 & 0.06 & 0.00 & 0.0 \\
2 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
3 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
4 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
5 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
6 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
7 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
8 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
9 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
10 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
11 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
12 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
13 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
14 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
15 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
16 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
17 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
18 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
19 & 0.0320 & 0.0000 & 0.06 & 0.00 & 0.0 \\
\bottomrule
\end{tabular}
\label{tab:ph_iters}
\end{table}
...
\midrule
Fractional memory & 0.8 & 0.5 & (1,0) & 1340.6 & 187.5 & 493.7 & 0.0 & 10.2 & 10.2 & -78.2 \\
First-order memory & 1.0 & 0.5 & (1,0) & 1342.2 & 188.5 & 495.8 & 0.0 & 10.6 & 10.2 & -78.1 \\
No operational memory & 0.8 & 0.0 & (1,0) & 1325.9 & 177.7 & 474.1 & 0.0 & 6.6 & 10.0 & -79.7 \\
\bottomrule
\end{tabular}
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Parameter recovery for one synthetic noisy realization. The estimated parameters lie near the generating values for this run. The profile loss surface does not establish structural identifiability or estimator consistency.}
...
allowances can be purchased, high-emission shipment and mode-use decisions remain feasible at a
...
Independent replication of the supplementary archive remains necessary until the complete experimental
archive is independently reproducible.

% =====================================================================
\section{Data Availability Statement}
The source code, scenario-tree data, solver logs, generated tables, and replication scripts accompanying this computational study are permanently archived in the supplementary repository (DOI: 10.5281/zenodo.21649028, version 1.0.1). The archive contains a \texttt{MANIFEST.txt} and cryptographic \texttt{SHA256SUMS.txt} to ensure integrity. The complete set of tables and figures presented in Section 7 can be exactly reproduced by executing the \texttt{generate\_all\_tables.py} script provided at the root of the archive.
