...
\begin{itemize}
  ...
  \item \(p_t^b\geq0\): unit price of purchased allowances;
  \item \(p_t^s\geq0\): unit revenue from sold allowances;
...
\begin{table}[ht]\small
\centering
\caption{Explicit four-scenario tree structure defining parent-child relationships, conditional probabilities, and nodal demands. The four-scenario tree is manually specified for implementation checking. It is not generated by the clustering and nested-distance reduction protocol of Section 3.4; consequently, no tree-reduction error is reported for this toy instance.}
\begin{tabular}{llrrr}
\toprule
...
\midrule
$\omega_{0}$ & $n_{1}$ & $n_{3}$ & 0.25 \\
$\omega_{1}$ & $n_{1}$ & $n_{4}$ & 0.25 \\
$\omega_{2}$ & $n_{2}$ & $n_{5}$ & 0.25 \\
$\omega_{3}$ & $n_{2}$ & $n_{6}$ & 0.25 \\
\bottomrule
\end{tabular}
